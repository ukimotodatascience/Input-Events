...
def parse_args() -> tuple[str, str, str, str, int, int]:
    ...
    name = sys.argv[3] if len(sys.argv) > 3 else os.getenv("NAME", "ニュース")
    fetch_mode = (
        (sys.argv[4] if len(sys.argv) > 4 else os.getenv("FETCH_MODE", "hours"))
        .strip()
        .lower()
    )
    hours = int(sys.argv[5] if len(sys.argv) > 5 else os.getenv("HOURS", "1"))
    diff_days = int(sys.argv[6] if len(sys.argv) > 6 else os.getenv("DIFF_DAYS", "1"))
    ...
    return feed_url, webhook_url, name, fetch_mode, hours, diff_days


def run():
    feed_url, webhook_url, name, fetch_mode, hours, diff_days = parse_args()
